...
public class DataProcessorSpark 
{
	static int counter=1;
	private JavaSparkContext sc;
	private HBaseDBManager db;
	private HashMap<String,String> mapKeywords;
	public DataProcessorSpark() throws IOException
	{
	...
	public void processData(String key, List<String> l) throws IOException
	{
		JavaRDD<String> list = this.sc.parallelize(l).flatMap(line -> Arrays.asList(line.toUpperCase().split("RT"))).filter(line ->!line.isEmpty());
		JavaPairRDD<String,Tweet>  tweetResults=list.mapToPair(new MyPairFunction(key,this.mapKeywords)).sortByKey();
		this.db.WriteTweetAnalysis(key,tweetResults.values());
	}
	
	static class MyPairFunction implements PairFunction<String, String, Tweet>,Serializable
	{
		private String key;
